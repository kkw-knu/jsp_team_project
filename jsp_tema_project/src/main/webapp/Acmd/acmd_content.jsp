...
<body>
    ...
    <div align="left"><br>
	<c:if test="${empty user_id }">
			<button class="btn btn-primary btn-sm" onclick="location.href='acmd.ac?pageNum=${pageNum}'">숙박업소 목록</button>
	</c:if>
    <c:if test="${not empty user_id }">
    	<c:if test="${user_id=='master' }">
    		<button class="btn btn-primary btn-sm" onclick="location.href='updateForm.ac?acmd_num=${acmd_num}&pageNum=${pageNum}'">수정</button>
				...
				<!-- 글 수정 모달 -->
									...
									<div class="panel-group">
										<div class="panel panel-success" style="margin-top: 10px;">
											...
											<div class="panel-body">
												<%-- form --%>
												<form class="form-horizontal" action="review1update.re" method="post" >
					...
					<div class="modal fade" id="myModal_delete${review.review_num }" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog">
							...
							<form action="review1delete.re" method="post">
								<input type="hidden" name="review_num" value="${review.review_num }">
								<input type="hidden" name="pageNum" value="${pageNum }">
								<input type="hidden" name="acmd_num" value="${acmd_num }">
								<input type="hidden" name="pageNum1" value="${pageNum1 }">
	...
	<div align="center">
		<c:if test="${startPage1 > PAGE_PER_BLOCK}">
			<button class="btn btn-default btn-xs" onclick="location.href='content.ac?acmd_num=${acmd_num}&pageNum=${pageNum }&pageNum1=${startPage1 - 1}'">이전</button>
		</c:if>
		<c:forEach var="i" begin="${ startPage1}" end="${ endPage1}">
			<button class="btn btn-default btn-xs" onclick="location.href='content.ac?acmd_num=${travel_num}&pageNum=${pageNum }&pageNum1=${i}'">${i }</button>
		</c:forEach>
		<c:if test="${endPage1 < totalPage1} }">
			<button class="btn btn-default btn-xs" onclick="location.href='content.ac?acmd_num=${travel_num}&pageNum=${pageNum }&pageNum1=${ endPage1 + 1}'">다음</button>
		</c:if>
	</div>
					...
					<div class="modal-body">
						<div class="panel-group">
							<div class="panel panel-success" style="margin-top: 10px;">
								...
								<div class="panel-body">
									<%-- form --%>
									<form class="form-horizontal" role="form" action="review1write.re" method="post">
									<input type="hidden" name="review_num" value="0">
									<input type="hidden" name="pageNum" value="${pageNum }">
									<input type="hidden" name="acmd_num" value="${acmd_num }">
									<input type="hidden" name="pageNum1" value="${pageNum1 }">
									<input type="hidden" name="review_id" value="${user_id }">
									<input type="hidden" name="review_acmd" value="${acmd.acmd_name }">
										<div class="form-group">
											<label class="control-label col-sm-2">작성자(ID):</label>
											<div class="col-sm-10">
										...
										<div class="form-group">
											<label class="control-label col-sm-2" for="pwd">숙박업소 :</label>
											<div class="col-sm-10">
												<input type="text" class="form-control" name="review_acmd" value="${acmd.acmd_name }" disabled>
											</div>
										</div>
										<div class="form-group">
											<label class="control-label col-sm-2" for="review_title">제목 :</label>
		...
		<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<form action="deleteAction.ac" method="post">
						<input type="hidden" name="acmd_num" required="required" value="${acmd.acmd_num}">
