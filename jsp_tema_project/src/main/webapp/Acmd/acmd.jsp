...
<body>
    ...
    <div>
    <br><h1 style="text-align:center; font-size:40px;">숙박 목록</h1><br>
    <div class="travelbox">
    ...
    <div align="center">
		<c:if test="${startPage > PAGE_PER_BLOCK}">
			<button class="btn btn-default btn-xs" onclick="location.href='acmd.ac?pageNum=${startPage - 1}'">이전</button>
		</c:if>
		<c:forEach var="i" begin="${ startPage}" end="${ endPage}">
			<button class="btn btn-default btn-xs" onclick="location.href='acmd.ac?pageNum=${i}'">${i }</button>
		</c:forEach>
		<c:if test="${endPage < totalPage} }">
			<button class="btn btn-default btn-xs" onclick="location.href='acmd.ac?pageNum=${ endPage + 1}'">다음</button>
		</c:if>
		<c:if test="${not empty user_id }">
	    	<c:if test="${user_id=='master' }">
			<br><br><button class="btn btn-primary btn-sm" onclick="location.href='writeForm.ac?acmd_num=0&pageNum=1'">숙박업소등록</button>
			</c:if>
		</c:if>
		</div>
    </div><!--메인 div-->
