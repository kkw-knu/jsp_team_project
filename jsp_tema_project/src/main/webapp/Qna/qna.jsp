...
<body>
	...
	<tr><th>번호</th><th>	제목</th><th>작성자</th><th>이메일</th><th>작성일</th><th>조회수</th></tr>
			...
			<c:forEach var="qna" items="${list }">
				...
				<c:if test="${qna.qna_del != 'y' }">
					<td title="${qna.qna_content }">
						...
						<a href="content.jsp?num=${qna.qna_num}&pageNum=${ currentPage}">
						${qna.qna_title}</a>
		...
		<c:if test="${startPage > PAGE_PER_BLOCK} }">
			<button onclick="location.href='qna.do?pageNum=${startPage - 1}'">이전</button>
		</c:if>
		<c:forEach var="i" begin="${ startPage}" end="${ endPage}">
			<button onclick="location.href='qna.do?pageNum=${i}'">${i }</button>
		</c:forEach>
		<!-- 	보여줄 것이 아직 남아있다 -->
		<c:if test="${endPage < totalPage} }">
			<button onclick="location.href='qna.do?pageNum=${ endPage + 1}'">다음</button>
		</c:if>
